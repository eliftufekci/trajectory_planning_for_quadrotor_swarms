...
BezierCurve::BezierCurve(int degree, int continuity_level, const Environment* env, const SubdividedSchedule& schedule)
    : D(degree), C(continuity_level), environment(env), subdividedSchedule(schedule) {
    fill_lookup_table();
}

void BezierCurve::fill_lookup_table(){
...
Eigen::MatrixXd BezierCurve::get_sub_Q_mono_matrix(int c, double T_piece){
...
Eigen::MatrixXd BezierCurve::get_total_Q_mono_matrix(const std::vector<double>& user_parameter, int K, double T_piece){
...
Eigen::MatrixXd BezierCurve::get_B_matrix(double T_piece) {
...
Eigen::MatrixXd BezierCurve::get_total_B_matrix(int K, double T_piece){
...
double BezierCurve::combinations(int n, int i){
    if (n < i || i < 0) return 0;
    return (double)factorial_lookup[n] / (factorial_lookup[i] * factorial_lookup[n-i]);
}

std::vector<Eigen::Vector3d> BezierCurve::compute_control_points(const SafePolyhedron& safe_polyhedron,
...
std::vector<std::vector<Eigen::Vector3d>> BezierCurve::compute(  const SafePolyhedron& safe_polyhedron,
                                                    const std::vector<double>& user_parameter, // gamma_c weights
                                                    int K, // number of pieces
                                                    double T_total,
...
std::vector<Eigen::Vector3d> BezierCurve::linear_fallback(   const Eigen::Vector3d& start_pos, 
