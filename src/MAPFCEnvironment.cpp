...
MAPFCEnvironment::MAPFCEnvironment(const Graph& graph, const ConflictAnnotation& annotation)
...
void MAPFCEnvironment::setLowLevelContext(size_t agentIdx, const Constraints* constraints) {
    m_agentIdx = agentIdx;
...
int MAPFCEnvironment::admissibleHeuristic(const State& s) {
    return static_cast<int>((graph.vertices[s.vertex_id].pos - graph.vertices[graph.goal_vertices[m_agentIdx]].pos).norm());
}

bool MAPFCEnvironment::isSolution(const State& s) {
    return s.vertex_id == graph.goal_vertices[m_agentIdx] && s.time > m_lastGoalConstraint;
}

void MAPFCEnvironment::getNeighbors(const State& s, std::vector<libMultiRobotPlanning::Neighbor<State, Action, int> >& neighbors) {
    neighbors.clear();
...
bool MAPFCEnvironment::getFirstConflict( const std::vector<libMultiRobotPlanning::PlanResult<State, Action, int> >& solution, Conflict& result) {
    int max_t = 0;
...
void MAPFCEnvironment::createConstraintsFromConflict(
...
// low-level
int MAPFCEnvironment::focalStateHeuristic(
...
// low-level
int MAPFCEnvironment::focalTransitionHeuristic(
...
// Count all conflicts
int MAPFCEnvironment::focalHeuristic(
    const std::vector<libMultiRobotPlanning::PlanResult<State, Action, int> >& solution) {
    int numConflicts = 0;
...
void MAPFCEnvironment::onExpandHighLevelNode(int /*cost*/) { m_highLevelExpanded++; }

void MAPFCEnvironment::onExpandLowLevelNode(const State& /*s*/, int /*fScore*/, int /*gScore*/) { m_lowLevelExpanded++; }

int MAPFCEnvironment::highLevelExpanded() { return m_highLevelExpanded; }

int MAPFCEnvironment::lowLevelExpanded() const { return m_lowLevelExpanded; }



State MAPFCEnvironment::getState(size_t agentIdx, const std::vector<libMultiRobotPlanning::PlanResult<State, Action, int> >& solution, size_t t) {
...
bool MAPFCEnvironment::transitionValid(int time, int edge_id) {
