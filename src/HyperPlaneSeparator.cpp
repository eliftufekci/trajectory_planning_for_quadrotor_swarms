...
SafePolyhedron HyperPlaneSeparator::compute(){
    ...
    // For each `(i, j, k)` triple, compute an `(n, d)` pair: robot `i` stays
    // on the `n^T x >= d` side at timestep `k`, while robot `j` stays on
    // the `n^T x <= d` side.
    // robot - robot
    for(int k=0; k < subdividedSchedule.K; k++){
        for(int i=0; i < positions.size(); i++){ // robot i
            for(int j=i+1; j< positions.size(); j++){ // robot j                    
                ...
                // Add the opposite plane for the other agent as well.
    ...
    for(int k = 0; k < subdividedSchedule.K; k++){
        for(int i = 0; i < positions.size(); i++){
            ...
            // Find obstacles near the segment from the octree.
...
SafePolyhedron HyperPlaneSeparator::compute(const std::vector<std::vector<std::vector<Eigen::Vector3d>>>& samples) {
    ...
    for(size_t k = 0; k < K; ++k) {
        for(size_t i = 0; i < num_agents; ++i) {
            if (S == 0) continue; // Skip this step if there are no sample points.

            // Compute the bounding box (AABB) containing the trajectory segment samples.
            ...
            // Use the octree to efficiently find obstacles near this box.
...
HyperPlane HyperPlaneSeparator::computeSVM(const Eigen::MatrixXd& points_i, const Eigen::MatrixXd& points_j, bool is_obstacle){
    ...
    // --- 1. Hessian matrix (P): minimize 0.5 * x'Px ---
    Eigen::SparseMatrix<double> P(n_vars, n_vars);
    if(is_obstacle){
        // For obstacles, maximize the margin using the robot radius.
        ...
        // For robot-robot separation, use ellipsoid radii.
...
std::vector<size_t> HyperPlaneSeparator::findNearbyObstacles(const Eigen::Vector3d& p_start, const Eigen::Vector3d& p_end, double margin) {
    // 1. Compute the segment bounding box and add margin.
    ...
    // 2. Query occupied nodes intersecting this box in the octree.
    for(auto it = environment.tree->begin_leafs_bbx(bbx_min, bbx_max); it != environment.tree->end_leafs_bbx(); ++it){
        ...
        for (size_t obs_idx = 0; obs_idx < environment.obstacles.size(); ++obs_idx) {
            if (environment.obstacles[obs_idx].contains(node_center)) {
                ...
                // Assume a node center can belong to only one obstacle and exit
                // the loop. This improves efficiency.
                break;
            }
        }
    }
